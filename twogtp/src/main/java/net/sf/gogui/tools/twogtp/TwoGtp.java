    ...
    extends GtpEngine
{
            ...
            m_black.setIOCallback(new GtpClient.IOCallback()
                {
                    ...
                    {
                        appendDebugToCommentBuffer(GoColor.BLACK, s);
                    }
            ...
            m_white.setIOCallback(new GtpClient.IOCallback()
                {
                    ...
                    public void receivedStdErr(String s)
                    {
                        appendDebugToCommentBuffer(GoColor.WHITE, s);
                    }
    ...
    {
        m_debugToCommentBuffer.get(GoColor.BLACK).setLength(0);
        m_debugToCommentBuffer.get(GoColor.WHITE).setLength(0);
    }
        ...
        throws ErrorMessage
    {
        ...
        double realTimeBlack = m_realTime.get(GoColor.BLACK);
        double realTimeWhite = m_realTime.get(GoColor.WHITE);
        if (isAlternated())
        {
            resultBlack = inverseResult(resultBlack);
            resultWhite = inverseResult(resultWhite);
            resultReferee = inverseResult(resultReferee);
            realTimeBlack = m_realTime.get(GoColor.WHITE);
            realTimeWhite = m_realTime.get(GoColor.BLACK);
        }
    ...
    private void initGame(int size) throws GtpError
    {
        ...
        m_realTime.set(GoColor.BLACK, 0.);
        m_realTime.set(GoColor.WHITE, 0.);
        // Clock is not needed
        m_game.haltClock();
        m_resigned = false;
        if (m_openings != null)
